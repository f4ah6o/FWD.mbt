...
- workbench builder に
  - `schema_status`
  - `rollout_mode`
  を追加し、「設計中か」「review 中か」「released 済みか」と
  「simulation / shadow / assisted / enforced のどこか」を明示できるようにした
- validation / preview の下に rollout section を追加し、
  **今このモデルでどこまで実運用に進めるべきか**を deterministic に表示するようにした
- preset は次の流れをカバーする
  - `Draft -> Submitted`
...
さらに、`Released + shadow` という preset を通して、

- v6 workbench で設計・検証する
- frozen surface (`/v2.1`, timeline, metrics, observability) で shadow 運用を始める
- drift が減ったら assisted / enforced に進む

という **design-first だが operation-ready な導線**を具体化できた。
